...
//------------------------------------------------------------Geração da tabela-------------------------------------------------------------------------
    ...
    private function geraTabela($param = array()) {
        ...
        try {
            ...
            $comando = $pdo->query("SELECT * FROM cad_noticia WHERE " . $expre . " ORDER BY " . $param[0] . " LIMIT " . $param[1] . " OFFSET " . $param[2]);
            ...
            echo "<table class=priTbl>
                        ...
                        <div class=divPriTitTr>
                            ...
                            <div class=divTitTr><a href=?tbl=2>Titulo</a></div>
                            <div class=divTitTr><a href=?tbl=5>Categoria</a></div>
                            <div class=divTitTr><a href=?tbl=6>Data Cadastro</a></div>
                            <div class=divTitTr>Imagem</div>
            ...
            foreach ($listar as $list):
                echo '<tr>
                        <div class=divPriTr>
                            <div class= "divtr divTrCodCatDat">' . $list['id'] . '</div>
                            <div class= "divtr divTrTitNoti">' . $list['titulo'] . '</div>
                            <div class= "divtr divTrCodCatDat">' . $list['categoria'] . '</div>
                            <div class= "divtr divTrCodCatDat">' . date('d/m/Y H:i:s', strtotime($list['dt_cadastro'])) . '</div>
                            <div class=divtr><img src=../imagensNoticias/' . $list['imagem'] . ' width=70 height=80 alt=imgEditar/></div>
                            <div class=divtrOpr><a href=telaEditaNoticia.php?id=' . $list['id'] . '&categoria=' . $list['categoria'] . '> <img src= ../imagensSistema/imgEditar.jpg width=40 height=40 alt=sort_both/></a></div>
...
//--------------------------------------------------------------Rodape Paginação------------------------------------------------------------------------
            ...
            $sql = $pdo->query('SELECT COUNT(id) AS contagem FROM cad_noticia WHERE ' . $expre . ' LIMIT 500');
